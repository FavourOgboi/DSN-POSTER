...
# Load the Excel file
xl = pd.ExcelFile('./DESIGN OF EXPERIMENT HCL.xlsx')
...
df = df.dropna(subset=['HCl_M', 'Inhibitor_ppm', 'Temp_C'])

# Convert to appropriate types
df['HCl_M'] = df['HCl_M'].astype(float)
df['Inhibitor_ppm'] = df['Inhibitor_ppm'].astype(int)
df['Temp_C'] = df['Temp_C'].astype(int)

# Reorder columns
df = df[['Sample', 'HCl_M', 'Inhibitor_ppm', 'Temp_C', 'E_corr_V', 'j_corr_A_cm2', 'I_corr_A', 'CR_mm_yr', 'PR_ohm']]
...
print("\nUnique values:")
print("HCl_M:", sorted(df['HCl_M'].unique()))
print("Inhibitor_ppm:", sorted(df['Inhibitor_ppm'].unique()))
print("Temp_C:", sorted(df['Temp_C'].unique()))

# Save cleaned data
output_dir = './cleaned data hcl/'
df.to_csv(output_dir + 'cleaned_HCL_tafel_data.csv', index=False)
print(f"\nCleaned data saved to '{output_dir}cleaned_HCL_tafel_data.csv'")
...
df_ocp_clean['HCl_M'] = parsed_ocp.apply(lambda x: x[0])
df_ocp_clean['Inhibitor_ppm'] = parsed_ocp.apply(lambda x: x[1])
df_ocp_clean['Temp_C'] = parsed_ocp.apply(lambda x: x[2])

# Drop failed parses
df_ocp_clean = df_ocp_clean.dropna(subset=['HCl_M'])

print(f"OCP cleaned shape: {df_ocp_clean.shape}")
df_ocp_clean.to_csv(output_dir + 'cleaned_HCL_OCP_data.csv', index=False)
print(f"OCP data saved to '{output_dir}cleaned_HCL_OCP_data.csv'")
...
df_lsv_clean['HCl_M'] = parsed_lsv.apply(lambda x: x[0])
df_lsv_clean['Inhibitor_ppm'] = parsed_lsv.apply(lambda x: x[1])
df_lsv_clean['Temp_C'] = parsed_lsv.apply(lambda x: x[2])

# Drop failed
df_lsv_clean = df_lsv_clean.dropna(subset=['HCl_M'])

print(f"LSV cleaned shape: {df_lsv_clean.shape}")
df_lsv_clean.to_csv(output_dir + 'cleaned_HCL_LSV_data.csv', index=False)
print(f"LSV data saved to '{output_dir}cleaned_HCL_LSV_data.csv'")
